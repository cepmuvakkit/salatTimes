...
public class Equatorial {
   ...
   public Horizontal Equ2Topocentric (double longitude, double latitude, double Height, double jd, double ΔT)
		...
		double αPrime =Math.toRadians(α)+Δα;
         ...
         horizontal.setAzimuth(Math.toDegrees(MATH.atan2(Math.sin(HPrime), Math.cos(HPrime)*Math.sin(ϕ)-Math.tan(δPrime)* Math.cos(ϕ))+Math.PI));
         horizontal.setElevation(Math.toDegrees( MATH.asin (Math.sin(ϕ)*Math.sin(δPrime)+ Math.cos(ϕ) *Math.cos(δPrime)*Math.cos (HPrime))));
         return horizontal;
